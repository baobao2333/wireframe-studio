...
安装器尚无 Windows Authenticode 代码签名证书。Windows 可能显示“未知发布者”或 SmartScreen 提示。安装前核对仓库及 Release 的 SHA-256；不要安装来源不明的文件。
...
- 运行时更新：下载 NSIS 安装包，核对已签名清单中的版本与 SHA-256，用户确认后安装并重新启动。

Ed25519 更新签名不等同于 Windows Authenticode。发布私钥不在仓库、安装器或 CI 中。GitHub 不可用时仍可编辑本地工程。
